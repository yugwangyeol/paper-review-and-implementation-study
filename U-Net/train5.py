...
if mode == 'train':
    ...
    laoder_train = DataLoader(dataset_train,batch_size=batch_size,shuffle=True,num_workers=8)
...
def merge_images(top_left, top_right, bottom_left, bottom_right):
    merged_images = np.concatenate((top_left, top_right), axis=2)
    bottom = np.concatenate((bottom_left, bottom_right), axis=2)
    merged_images = np.concatenate((merged_images, bottom), axis=1)
...
def main():
    ...
    if mode == 'train':
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for batch, data in enumerate(laoder_train, 1):
                # forward pass
                ...
                input_images = split_images(input)
                output_images = []

                #print(input_images[0].size())

                for image in input_images:
                    image = torch.nn.functional.pad(image, (94,94,94,94), 'reflect',0)
                    print(image.size())
                    output_img = net(image)
                    print(output_img.size())
                    output_images.append(output_img)
                
                output = merge_images(*output_images)
